...
function SignUp() {
  ...
  const { api, setUserData, refreshCurrentUser } = useContext(userDataContext)
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      const nextUser = result.data?.user || await refreshCurrentUser()
      setUserData(nextUser)
      navigate("/customize")
    } catch (error) {
      console.log(error)
      setUserData(null)
      setErr(error?.response?.data?.message || "Unable to create your account right now.")
    } finally {
      setLoading(false)
    }
  }
